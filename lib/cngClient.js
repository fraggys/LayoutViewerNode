...
clientSock.on('timeout', function (data) {
    console.log("Socket timeout " + data);
});

clientSock.on('drain', function (data) {
    console.log("Received drain on the socket " + data);
});

clientSock.on('close', function (data) {
    console.log("This socket is closed " + data);
});

clientSock.on('data', function (data) {
    var str = data.toString();
    console.log("4) Data Received on Socket");
    buffer = buffer + str;
            ...
            for (var i = 0; i < received.Root.Barco.length; i += 1) {
                ...
                if (data.Acknowledge) {
                    ...
                    if (data.Notify[0].$.commandNumber === "-1") {
                        ...
                        console.log("and the result received is :");
